...
def upgrade():
    ...
    # NOTE: keep only FK within renovation module; do NOT reference users/stores
    # because this repo's migration chain doesn't create those base tables.
    op.create_table(
        'renovation_tasks',
        ...
        sa.Column('priority', sa.Enum('URGENT', 'HIGH', 'MEDIUM', 'LOW', name='renovationtaskpriority'), nullable=True,
                  comment='优先级'),
        sa.Column('status', sa.Enum('PENDING', 'PROCESSING', 'AWAITING_VERIFICATION', 'COMPLETED', 'CLOSED', 'REJECTED',
                                    name='renovationtaskstatus'), nullable=True, comment='任务状态'),
        ...
        sa.Column('verification_result', sa.Enum('PASSED', 'FAILED', name='verificationresult'), nullable=True,
                  comment='验收结果'),
    ...
    op.create_table(
        ...
        sa.Column('task_id', sa.Integer(), nullable=False, comment='任务ID'),
        sa.Column('action',
                  sa.Enum('CREATE', 'UPDATE', 'UPLOAD_EVIDENCE', 'SUBMIT_FOR_VERIFICATION', 'VERIFY', 'REJECT', 'CLOSE',
                          name='renovationrecordaction'), nullable=False, comment='操作类型'),
