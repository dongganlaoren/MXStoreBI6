...
Revision ID: 6d52051b9c5f
Revises: 7b4517cb3530
Create Date: 2026-01-30 10:39:02.725955
...
revision = '6d52051b9c5f'
down_revision = '7b4517cb3530'
branch_labels = None
depends_on = None


def _table_exists(table_name: str) -> bool:
    """Return True if table already exists in current database."""
    bind = op.get_bind()
    insp = sa.inspect(bind)
    try:
        return insp.has_table(table_name)
    except Exception:
        # If inspection fails for dialect reasons, be conservative and
        # assume it doesn't exist so Alembic will try to create it.
        return False


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    # Production-safety: some deployments may have tables created manually
    # (or via earlier drift). For those cases, skip create_table to avoid
    # failing the whole upgrade with "Table already exists".

    if not _table_exists('email_report_config'):
        op.create_table('email_report_config',
                        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
                        sa.Column('role', sa.Enum('ADMIN', 'HEAD_MANAGER', 'FINANCE', 'BRANCH_MANAGER', 'EMPLOYEE',
                                                  name='roletype'), nullable=False, comment='角色'),
    ...
    if not _table_exists('email_task_log'):
        op.create_table('email_task_log',
                        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
                        sa.Column('task_type', sa.Enum('daily', 'weekly', 'monthly', name='emailtasktype'),
                                  nullable=False,
                                  comment='任务类型'),
                        ...
                        sa.Column('status', sa.Enum('success', 'partial_fail', 'fail', name='emailtaskstatus'),
                                  nullable=False, comment='发送结果状态'),
    ...
    if not _table_exists('stores'):
        op.create_table('stores',
                        ...
                        sa.Column('third_party_platform', sa.Boolean(), nullable=True,
                                  comment='是否开启第三方外卖平台'),
    ...
    if not _table_exists('users'):
        op.create_table('users',
                        ...
                        sa.Column('role', sa.Enum('ADMIN', 'HEAD_MANAGER', 'FINANCE', 'BRANCH_MANAGER', 'EMPLOYEE',
                                                  name='roletype'), nullable=True, comment='用户角色'),
                        ...
                        sa.Column('id_card_copy', sa.String(length=255), nullable=True,
                                  comment='身份证复印件文件路径（受控访问，避免隐私泄露）'),
    ...
    if not _table_exists('daily_sales'):
        op.create_table('daily_sales',
                        sa.Column('theoretical_total', sa.Float(), nullable=True,
                        ...
                        sa.Column('pos_total', sa.Float(), nullable=True,
                                  comment='店铺理论营业额 (T0) = 现金收入 + 电子支付收入 + 外卖收入 + 代金券使用金额'),
                        sa.Column('electronic_actual_arrival', sa.Float(), nullable=True,
                                  comment='电子支付实际入账金额 (EA)'),
                        ...
                        sa.Column('electronic_difference', sa.Float(), nullable=False,
                        ...
                        sa.Column('takeaway_info_completed', sa.Boolean(), nullable=False,
                                  comment='第二步(外卖)是否完成'),
                        sa.Column('actual_arrival_info_completed', sa.Boolean(), nullable=False,
                                  comment='实际入账金额录入是否完成'),
                        sa.Column('is_submitted', sa.Boolean(), nullable=False, comment='是否已最终提交给财务'),
                        sa.Column('financial_check_status', sa.Enum('PENDING', 'APPROVED', name='financialcheckstatus'),
                                  nullable=False, comment='财务核对状态（仅PENDING/APPROVED）'),
                        sa.Column('created_at', sa.DateTime(), nullable=True, comment='创建时间'),
                        sa.Column('updated_at', sa.DateTime(), nullable=True, comment='更新时间'),
                        sa.ForeignKeyConstraint(['store_id'], ['stores.store_id'], ),
                        sa.ForeignKeyConstraint(['user_id'], ['users.user_id'], ),
