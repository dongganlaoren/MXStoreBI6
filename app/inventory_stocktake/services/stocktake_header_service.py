...
def save_draft_batch(*, store_id: str, check_date: date, operator: Optional[str], items: List[dict]) -> DraftSaveResult:
    """Unified save for draft: upsert many draft rows under a header.
    ...
    for it in items:
        try:
            ...
            existing = MXInventoryDraft.query.filter_by(
                store_id=store_id,
                check_date=check_date,
                material_code=material_code,
            ).first()
            if existing is None:
                existing = MXInventoryDraft(
                    store_id=store_id,
                    check_date=check_date,
                    material_code=material_code,
            ...
            if "Unknown column" in msg and "header_id" in msg:
                raise ValidationError(
                    "数据库缺少字段 header_id：请先执行数据库迁移（flask db upgrade）后再保存草稿") from e
            last_error = msg
            fail += 1
        except Exception as e:
            last_error = str(e)
    ...
    db.session.commit()
    return DraftSaveResult(saved=ok, failed=fail, message=f"保存完成：成功{ok}条，失败{fail}条")
...
def commit_stocktake_with_items(*, store_id: str, check_date: date, operator: Optional[str],
                                items: List[dict]) -> CommitWithItemsResult:
...
def commit_stocktake(*, store_id: str, check_date: date, operator: Optional[str]) -> CommitResult:
    ...
    # must have at least one draft detail
    drafts = MXInventoryDraft.query.filter_by(store_id=store_id, check_date=check_date).all()
    if not drafts:
        raise ValidationError("没有盘点明细，无法提交")
...
def load_draft_details(*, store_id: str, check_date: date) -> List[dict]:
    ...
    try:
        ...
        # e.g. (pymysql.err.ProgrammingError) (1146, "Table 'xxx.mx_inventory_draft' doesn't exist")
        msg = str(e)
