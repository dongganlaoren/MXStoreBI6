...
class DailySalesAttachments(db.Model):
    ...
    report_id = db.Column(
        db.Integer,
        db.ForeignKey("daily_sales.report_id", ondelete="CASCADE"),
        nullable=False,
        comment="日报ID",
    )
    file_path = db.Column(
    ...
    uploaded_at = db.Column(
        db.DateTime, default=datetime.now, comment="上传时间"
    )

    def __repr__(self):
        # 日志记录示例
        current_app.logger.info(
            f"[附件模型] 查询凭证: id={self.attachment_id}, "
            f"类型={self.attachment_type}"
        )
        return f"<DailySalesAttachments {self.attachment_id}>"
