...
class User(UserMixin, db.Model):
    ...
    employee_number = db.Column(
        db.Integer, nullable=True, comment="员工编号（店铺编号+三位序号）"
    )
    """
    系统用户模型（已合并员工档案信息）
    """
    ...
    username = db.Column(
        db.String(64), unique=True, nullable=False, comment="登录用户名"
    )
    password_hash = db.Column(
        db.String(255), nullable=False, comment="登录密码哈希"
    )
    user_status = db.Column(
        db.Integer,
        nullable=False,
        default=1,
        comment="用户状态（1=活跃，0=禁用）",
    )
    last_login_time = db.Column(
        db.DateTime, default=datetime.now, comment="最近一次登录时间"
    )
    ...
    role = db.Column(
        db.Enum(RoleType), default=RoleType.EMPLOYEE, comment="用户角色"
    )
    ...
    id_card_number = db.Column(
        db.String(100), nullable=True, comment="身份证号"
    )
    bank_name = db.Column(db.String(100), nullable=True, comment="银行名称")
    bank_account_number = db.Column(
        db.String(100), nullable=True, comment="银行账号"
    )
    is_primary_contact = db.Column(
        db.Boolean, default=False, comment="是否为店铺主要联系人"
    )
