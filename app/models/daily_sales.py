...
class DailySales(db.Model):
    ...
    # 理论营收总金额
    theoretical_total = db.Column(
        db.Float,
        comment="理论营收(T2)=店铺理论营业额(T0)+第三方外卖平台收入(T1)"
        "-POS机小票里显示的代金券总金额-银行存款金额",
    )
    """
    每日营业额上报记录模型
    字段及计算公式详见《字段说明.md》
    ...
    report_date = db.Column(
        db.Date, nullable=False, index=True, comment="营业日期"
    )
    ...
    pos_total = db.Column(
        db.Float,
        comment="店铺理论营业额 (T0) = 现金收入 + 电子支付收入 + "
        "外卖收入 + 代金券使用金额",
    )

    # 实际入账
    electronic_actual_arrival = db.Column(
        db.Float, comment="电子支付实际入账金额 (EA)"
    )
    bank_deposit = db.Column(db.Float, comment="银行存款金额 (BC)")
    bank_fee = db.Column(db.Float, comment="银行存款手续费 (BF)")

    # 第三方外卖平台
    takeaway_amount = db.Column(
        db.Float,
        default=0.0,
        nullable=False,
    ...
    # 实际总营业额
    actual_sales = db.Column(
        db.Float,
        comment="实际总营业额(S)=第三方外卖平台收入(T1)+外卖收入+"
        "电子支付实际入账金额+银行存款金额",
    )

    # 误差
    total_error = db.Column(
        db.Float,
        comment="总误差(E)=电子支付实际入账金额+银行存款金额+银行存款手续费-"
        "POS机小票里显示的电子支付总金额-POS机小票里显示的现金总金额",
    )

    # 误差字段仅存储，默认0
    cash_difference = db.Column(
        db.Float,
        default=0.0,
        nullable=False,
        comment="POS现金收入误差(A)，仅存储，默认0",
    )
    electronic_difference = db.Column(
        db.Float,
        default=0.0,
        nullable=False,
        comment="POS电子支付误差(B)，仅存储，默认0",
    )

    remark = db.Column(db.String(255), comment="审核备注")

    # 步骤与状态
    pos_info_completed = db.Column(
        db.Boolean,
        default=False,
        nullable=False,
        comment="第一步(POS)是否完成",
    )
    takeaway_info_completed = db.Column(
        db.Boolean,
        default=False,
        nullable=False,
        comment="第二步(外卖)是否完成",
    ...
    actual_arrival_info_completed = db.Column(
        db.Boolean,
        default=False,
        nullable=False,
    ...
    def to_dict(self):
        ...
        return {
            ...
            "financial_check_status": (
                self.financial_check_status.value
                if self.financial_check_status
                else None
            ),
            "created_at": (
