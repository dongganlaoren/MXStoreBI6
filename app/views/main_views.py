    ...
    try:
        ...
        for store in stores:
            ...
            if has_t1 is None:
                ...
                t1_count = db.session.query(func.count(DailySales.report_id)).filter(
            ...
            t1 = None
            if has_t1:
                t1 = db.session.query(func.sum(DailySales.takeaway_amount)).filter(
                    DailySales.store_id == store.store_id,
                    DailySales.report_date >= first_day_of_month,
                    DailySales.financial_check_status == FinancialCheckStatus.APPROVED
                ).scalar() or 0
            actual = db.session.query(func.sum(DailySales.actual_sales)).filter(
                DailySales.store_id == store.store_id,
                DailySales.report_date >= first_day_of_month,
                DailySales.financial_check_status == FinancialCheckStatus.APPROVED
            ).scalar() or 0
            ...
            # 近一周有审核通过日报（不要求连续）
            last_7_days_q = db.session.query(
            ...
            last_7_days = [r[0] for r in last_7_days_q.order_by(DailySales.report_date.desc()).limit(7)]
            reports = []
            for d in reversed(last_7_days):
                ...
                reports.append({
                    ...
                    't1': float(day_data[1]) if (has_t1 and day_data[1] is not None) else None,
