...
from app.forms.reimbursement_forms import (
...
from app.models.reimbursement import ReimbursementAttachment, ReimbursementRequest
...
def list_requests():
    ...
    if status != "all":
        query = query.filter(
            ReimbursementRequest.status == getattr(ReimbursementStatus, status)
        )
    query = query.filter(
        ReimbursementRequest.created_at >= begin,
        ReimbursementRequest.created_at < end,
    )
    requests = query.order_by(ReimbursementRequest.created_at.desc()).all()

    return render_template(
        "reimbursement/list.html",
        requests=requests,
        category=category,
        status=status,
...
def create():
    ...
    if form.validate_on_submit():
        try:
            ...
            approver_id = (
                int(form.approver_id.data) if form.approver_id.data else None
            )
            ...
            for file in files:
                if file and file.filename:
                    ...
                    save_name = f"{req.request_id}_{int(datetime.utcnow().timestamp())}_{filename}"  # 防止重名，拼接request_id和时间戳
                    file_path = os.path.join(upload_folder, save_name)
                    file.save(file_path)
                    rel_path = os.path.relpath(
                        file_path, start=current_app.root_path
                    )
...
def detail(request_id):
    ...
    return render_template(
        "reimbursement/detail.html", req=req, attachments=attachments
    )
...
def list_all():
    ...
    if submitter:
        query = query.join(
            User, ReimbursementRequest.submitter_id == User.user_id
        )
        query = query.filter(
    ...
    if approver:
        query = query.join(
            User, ReimbursementRequest.approver_id == User.user_id
        )
        query = query.filter(
