...
def manage_list():
    ...
    role_val = getattr(current_user.role, 'value', None) if hasattr(current_user.role, 'value') else str(
        current_user.role)
    is_store_staff = role_val in ['BRANCH_MANAGER', 'EMPLOYEE', 'branch_manager', 'employee']

    # 可选门店列表（下拉）
    if is_store_staff:
        if getattr(current_user, 'store_id', None):
    ...
    if is_store_staff:
        if getattr(current_user, 'store_id', None):
            query = query.filter(DailySales.store_id == current_user.store_id)
            # 同步前端筛选显示为当前店铺
            if not store_id or store_id != current_user.store_id:
    ...
    # 默认按更新时间倒序
    query = query.order_by(DailySales.updated_at.desc())
...
def manage_report_create():
    ...
    if form.validate_on_submit():
        ...
        daily_sales = DailySales(
            ...
            pos_income=form.pos_income.data or 0,
            voucher_amount=form.voucher_amount.data or 0,
            takeaway_amount=(form.takeaway_amount.data or 0) if show_takeaway else 0,
            electronic_actual_arrival=form.electronic_actual_arrival.data or 0,
            bank_deposit=form.bank_deposit.data or 0,
            bank_fee=form.bank_fee.data or 0
        )
